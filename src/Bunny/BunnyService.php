...
class BunnyService {
  ...
  public function thumbnailUrl(string $guid,int $ttl=86400): string {
    $exp=time()+$ttl;
    if($this->tokenKey){
      $token=hash('sha256',$this->tokenKey.$guid.$exp);
      if($this->cdnHost) return "https://{$this->cdnHost}/{$guid}/thumbnail.jpg?token=$token&expires=$exp";
  ...
  public function syncToDb(): int {
    ...
    do{
      ...
      $items=$resp['items']??$resp??[]; if(!is_array($items)||empty($items)) break;
      foreach($items as $v){
        $guid=$v['guid']??''; if(!$guid) continue;
        $title=$v['title']??'Untitled'; $col=$v['collectionId']??null;
        ...
        else { \MarineVideoPortal\Core\Database::exec("INSERT INTO videos_meta (guid,collection_id,title,watermark_mode,custom_order) VALUES (?,?,?,?,0)",[$guid,$col,$title,'default']); }
        $count++;
      }
      if(count($items)<100) break; $page++; if($page>50) break;
    }while(true);
    return $count;
  }
}
