...
class BundlePage {
  public static function render(string $id){
    ...
    // Privacy: do NOT show email at all
    ...
    foreach($shares as $s){
      ...
      echo "<div class='card'><h3>$title</h3><p style='color:#888'>".($expired?"Expired":"Available until ".htmlspecialchars(date('M j',strtotime($s['expires_at']))))."</p><a href='/s/".htmlspecialchars($s['token'])."'><button>".($expired?"Unavailable":"Watch")."</button></a></div>";
    }
