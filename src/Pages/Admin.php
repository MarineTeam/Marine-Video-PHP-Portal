...
class Admin {
  public static function render(){
    ...
    if($tab==='videos'){
      echo '<div class="tab"><h2>Videos ('.Video::count().')</h2><form method="post"><input type="hidden" name="action" value="sync_bunny"><button>Sync from Bunny.net (Import)</button></form><table><tr><th>Title</th><th>Actions</th></tr>';
      foreach($videos as $v){ echo '<tr><td><form method="post" style="display:flex;gap:4px"><input type="hidden" name="action" value="update_video"><input type="hidden" name="guid" value="'.$v['guid'].'"><input name="title" value="'.htmlspecialchars($v['title']).'" style="width:220px"><input name="custom_order" value="'.$v['custom_order'].'" style="width:50px"><select name="watermark_mode"><option value="default">Default</option><option value="none">None</option></select><button>Save</button></form></td><td><a href="/watch/'.$v['guid'].'" target="_blank">Watch</a> <form method="post" style="display:inline"><input type="hidden" name="action" value="delete_video"><input type="hidden" name="guid" value="'.$v['guid'].'"><button style="background:#ef4444">Delete</button></form><hr><form method="post"><input type="hidden" name="action" value="create_share"><input type="hidden" name="guid" value="'.$v['guid'].'"><input name="email" placeholder="recipient@email.com" required style="width:160px"><input name="expiry" type="date" value="'.date('Y-m-d',strtotime('+30 days')).'"><label><input type="checkbox" name="notify" checked> Notify</label><button>Share</button></form></td></tr>'; }
      echo '</table></div>';
    } elseif($tab==='viewers'){ echo '<div class="tab"><h2>Viewers - only these can login to main portal (private /s/ links bypass)</h2><form method="post"><input type="hidden" name="action" value="add_viewer"><input name="email" placeholder="email" required><input name="name" placeholder="Name"><label><input type="checkbox" name="approved" checked> Approved</label><button>Add/Update</button></form><table><tr><th>Email</th><th>Approved</th><th>Last Seen</th></tr>'; foreach($viewers as $v){ echo '<tr><td>'.htmlspecialchars($v['email']).'</td><td>'.($v['is_approved']?'Yes':'No').'</td><td>'.htmlspecialchars($v['last_seen_at']??'').'</td></tr>'; } echo '</table><p style="color:#888">Set ENFORCE_VIEWER_LIST=true in .env to enforce. ADMIN_EMAILS always allowed.</p></div>'; }
