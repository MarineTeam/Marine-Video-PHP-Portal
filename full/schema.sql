...
-- ---------------------------------------------------------------------
-- Private, time-limited share links (distinct from viewer_grants above:
-- these are secret single-purpose URLs with an expiry, revocable, emailable
-- via Resend, for sending one piece of content to someone who may not need
-- a standing account grant — merged in from the flat video-portal engine).
